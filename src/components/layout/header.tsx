...
function DesktopDropdown({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "absolute left-1/2 top-full w-max -translate-x-1/2 pt-3 transition-all duration-200 ease-premium",
          open
            ? "translate-y-0 opacity-100"
            : "pointer-events-none -translate-y-1 opacity-0",
        )}
      >
        <ul className="glass-light w-80 origin-top overflow-hidden rounded-2xl border border-white/60 p-2 shadow-(--shadow-card-hover)">
          {items.map((item) => (
              <li key={item.key}>
                <Link
                  href={item.href}
